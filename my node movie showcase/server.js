...
app.get('/movie/:movieId', (req, res) => {
    ...
    res.render('pages/movies', { movies: getmovies().filter(function (movie) {
        return movie.id == id;
    })});
})
...
app.post('/search', (req, res) => {
    ...
    res.render('pages/search', {movies: searchMovies(text)})
})

app.post('/imdb', (req, res)=> {
    var year = req.body.year;
    var title = req.body.title;
    console.log(year, +  " is title", title)
    imdb(title,year, res);
})
...
function searchMovies(title) {
    ...
    return getmovies().filter(function (movie) {
      return movie.title.toLowerCase().indexOf(title) !== -1;
    })
}



function imdb(title, year, res) {
    ...
    var req = http.request({
        ...
        resp.on('end', ()=> {
            ...
            obj.id = obj.imdbID;
            obj.title = obj.Title;
            arrayOfMovies.push(obj);
            res.render('pages/search', {movies: [obj]})
        });
